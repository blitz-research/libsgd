...
void RenderQueue::addRenderOp(CBuffer* vbuffer,		  //
							  ...
							  uint32_t firstElement, bool shadows) {
	...
	if (shadows && rpassType == RenderPassType::opaque) {
		...
		m_renderOps[(int)RenderPassType::shadow].back().pipeline = getOrCreateRenderPipeline(RenderPassType::shadow, material, geometry, renderer);
	}
}
